class Dice:
    ...
    def up_front_to_right(self, top, front):
        if (top, front) in [(self.top, self.front), (self.front,
            ...
            return self.right
        if (top, front) in [(self.front, self.top), (self.bottom,
                                                     self.front), (self.back, self.bottom), (self.top, self.back)]:
            return self.left
        if (top, front) in [(self.left, self.front), (self.front,
                                                      self.right), (self.right, self.back), (self.back, self.left)]:
            return self.top
        if (top, front) in [(self.front, self.left), (self.right,
                                                      self.front), (self.back, self.right), (self.left, self.back)]:
            return self.bottom
        if (top, front) in [(self.top, self.left), (self.left, self.bottom),
                            (self.bottom, self.right), (self.right, self.top)]:
            return self.front
        if (top, front) in [(self.left, self.top), (self.bottom,
                                                    self.left), (self.right, self.bottom), (self.top, self.right)]:
            return self.back
